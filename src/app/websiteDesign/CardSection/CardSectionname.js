...
const CardSectionname = () => {
  ...
  useEffect(() => {
    ...
    const updateTransformOrigins = () => {
      elementsCCRef.current.forEach((element) => {
        if (element) {
          const rect = element.getBoundingClientRect();
          const x = rect.left + rect.width / 2;
          const y = rect.top + rect.height / 2;

          const resultCC = `${x}px ${y}px`;
          element.style.setProperty("transform-origin", resultCC);
        }
      });

      elementsTLRef.current.forEach((element) => {
        if (element) {
          const rect = element.getBoundingClientRect();
          const x = rect.left;
          const y = rect.top;

          const resultTL = `${x}px ${y}px`;
          element.style.setProperty("transform-origin", resultTL);
        }
      });
    };

    // Slight delay to ensure DOM elements are fully loaded
    const timeoutId = setTimeout(updateTransformOrigins, 100);
            ...
            <g className="code">
              ...
              <g className="circle-bg">
                ...
                >
                  <stop
                    offset="6.016400e-02"
                    style={{ stopColor: "#DB6B86" }}
                  />
                  <stop
                    offset="8.596202e-02"
                    style={{ stopColor: "#DC6D87" }}
                  />
                  <stop offset={1} style={{ stopColor: "#F7A6A5" }} />
                </linearGradient>
                <circle
          ...
          <svg className="icon-support" viewBox="0 0 297.8 338.3">
			...
			c-1.5-6.3,2.4-12.6,8.7-14.1c6.3-1.5,12.6,2.4,14.1,8.7C241.3,51,237.4,57.3,231.1,58.8z"
              ...
              <g id="XMLID_82_">
                ...
                >
                  <stop
                    offset="9.016400e-002"
                    style={{ stopColor: "#A62257" }}
                  />
                  <stop offset="0.4541" style={{ stopColor: "#86245E" }} />
                  <stop offset={1} style={{ stopColor: "#582768" }} />
                </linearGradient>
