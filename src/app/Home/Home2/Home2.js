...
const Home2 = () => {
  ...
  const counters = [
    {
      ...
      heading: "ANALYSIS",
      paragraph:
        "The process of gathering, examining, and interpreting requirements to understand the problem that needs to be solved.",
      bgColor: "bg-blue-400",
    },
    {
      key: "artists",
      ...
      heading: "DESIGN",
      paragraph:
        "Translating the requirements gathered during analysis into a detailed plan or blueprint for building the solution.",
      bgColor: "bg-teal-300",
    },
    {
      key: "shows",
      ...
      heading: "DEVELOPMENT",
      paragraph:
        "The actual creation of the product or system based on the design specifications.",
    ...
    },
    {
      ...
      heading: "SUPPORT",
      paragraph:
        "Ongoing maintenance and assistance provided to ensure the solution operates effectively after deployment.",
      ...
      prevBalls.map((ball) =>
        ball.id === id
          ? {
              ...ball,
              isDragging: true,
              offset: { x: event.clientX - ball.x, y: event.clientY - ball.y },
            }
          : ball
      )
    );
      ...
      prevBalls.map((ball) => {
        ...
        if (distance < BLUE_BALL_RADIUS + BALL_RADIUS) {
          ...
          return {
            ...ball,
            x:
              BLUE_BALL_POSITION.x +
              Math.cos(angle) * (BLUE_BALL_RADIUS + BALL_RADIUS),
            y:
              BLUE_BALL_POSITION.y +
              Math.sin(angle) * (BLUE_BALL_RADIUS + BALL_RADIUS),
          };
        }
  ...
  const handleMouseUp = () => {
    setBalls((prevBalls) =>
      prevBalls.map((ball) => ({ ...ball, isDragging: false }))
    );
  };
      ...
      updatedBalls.forEach((ball) => {
        if (!ball.isDragging) {
          ...
          const distanceToBlue = Math.sqrt(
            dxToBlue * dxToBlue + dyToBlue * dyToBlue
          );
          ...
          if (ball.x > window.innerWidth - BALL_RADIUS)
            ball.x = window.innerWidth - BALL_RADIUS;
        }
      });
  ...
  useEffect(() => {
    ...
    targets.forEach((target) => {
      animateCountUp(target, maxCount / 50); 
    });
  }, []);
      ...
      >
        ...
        >
          <div className="flex flex-col p-6 text-center border-b mt-32 border-gray-100 dark:border-gray-700 sm:border-0 sm:border-r">
            <dt
              className="order-2 mt-10 text-xl leading-6 text-gray-500 dark:text-gray-400"
              id="item-1"
            >
